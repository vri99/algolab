...
type item_key_value = tuple[str, int | None]
type item_list = list[item_key_value | None | Node]

# Init default array with max 10 cells
def init_list(size: int | None = None) -> item_list:
    default: item_list = [None] * 10
...
def calculate_total_items(current_list: item_list) -> int:
    # get all elements from the array that is not None
    total_items: list[item_key_value] = [i for i in current_list if i is not None]
    # get total number
    total_items_len: int = len(total_items)

    return total_items_len

def resize_list(list_to_resize: item_list, n_elements_to_add: int) -> None:
...
def add_items(list_to_add: item_list, items: list[item_key_value]) -> None:
    ...
    for item in items:
        ...
        current_item: item_key_value | None | Node = list_to_add[item_hash]
        ...
        if type(current_item) is Node:
            if current_item.find(item) is None:
                current_item.append(item)

    return None

def get_item(value: item_key_value, items: item_list) -> int | None:
    if value is None:
        return None
    ...
    if type(items[item_hash]) is Node:
        return current_item.find(value)
...
def hash_item(value: item_key_value, item_list_len: int) -> int:
